...
1. Find the key by position:

   ```bash
   nix run .#eep -- keymap.eep --dump      # layers in use
   nix run .#eep -- keymap.eep --dump 2    # one layer
   ```

   Each cell prints its position and current keycode, seven per line so the rows
   split at the ErgoDone's physical halves.

2. Assign the new keycode. This rewrites the checksum for you, so prefer it over
   editing the hex:

   ```bash
   nix run .#eep -- keymap.eep --set 0,45=KC_V --set 0,46=KC_C
   ```

   Keys are `KC_X`, bare `x`, or a raw byte `0x1b`. `--set` is repeatable.
...
There are eight layer slots of 84 keys (`6 x 14`) at `0x53`; layers 0-2 are in
use and 3-7 sit empty, so a new layer needs no firmware change — write keycodes
into a free slot and point an fn key at it. The fn keys are 32 sixteen-bit TMK
action codes at `0x13`.
